...
/* Constructors */
Particle::Particle(const float inVp0, const float inMp, const Vector2f &inXp,
...
Water::Water(const float inVp0, const float inMp, const Vector2f &inXp,
             const Vector2f &inVp, const Matrix2f &inBp)
...
Water::Water(Particle p) : Particle(p.Vp0, p.Mp, p.Xp, p.Vp, p.Bp) {
  Ap = 0.0f;
  Jp = 1.0f;
}

//
DrySand::DrySand(const float inVp0, const float inMp, const Vector2f &inXp,
                 const Vector2f &inVp, const Matrix2f &inBp)
    : Particle(inVp0, inMp, inXp, inVp, inBp) {
  Ap.setZeros();
...
DrySand::DrySand(Particle p) : Particle(p.Vp0, p.Mp, p.Xp, p.Vp, p.Bp) {
  Ap.setZeros();
...
//
Snow::Snow(const float inVp0, const float inMp, const Vector2f &inXp,
           const Vector2f &inVp, const Matrix2f &inBp)
    : Particle(inVp0, inMp, inXp, inVp, inBp) {
  Ap.setZeros();
...
//
Elastic::Elastic(const float inVp0, const float inMp, const Vector2f &inXp,
                 const Vector2f &inVp, const Matrix2f &inBp, const float inlam,
                 const float inmu, const float inr, const float ing,
                 const float inb)
    : Particle(inVp0, inMp, inXp, inVp, inBp) {
  Ap.setZeros();
  Fe.setIdentity();
...
Elastic::Elastic(Particle p) : Particle(p.Vp0, p.Mp, p.Xp, p.Vp, p.Bp) {
  Ap.setZeros();
  Fe.setIdentity();
}
...
void Water::ConstitutiveModel() {
  float dJp = -K_water * (1.0f / pow(Jp, GAMMA_water) -
                          1.0f); // Deformation gradient increment
...
void Snow::ConstitutiveModel() {
  ...
  Matrix2f dFe = 2 * mu * (Fe - Re) * Fe.transpose() +
                 lam * (Je - 1) * Je * Matrix2f(1, 0, 0, 1);
  Ap = dFe * Vp0;
}
