...
/* Constructors */
Border::Border(const int inType, const Vector2f &inNormal,
               const std::vector<Vector2f> &inX_corner) {
  type = inType;
  normal = inNormal;
  X_corner = inX_corner;
}
...
void Border::Collision(const Vector2f &node_coordinates,
                       Vector2f &node_velocity, std::vector<int> &collision,
                       const int b) {
  ...
  /* If not sticky, compute trial distance (after update of Xi). */
    ...
    float trial_distance = normal.dot(trial_position - X_corner[0]);
...
void Border::Friction(Vector2f &Vi_fri, const Vector2f &Vi_col,
                      const Vector2f &Vi) {
