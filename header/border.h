...
public:
  ...
  /* Constructors */
  Border(){};
  Border(const int inType, const Vector2f &inNormal,
         const std::vector<Vector2f> &inX_corner);
  ~Border(){};

  /* Functions */
  void Collision(
      const Vector2f &node_coordinates, // Apply collision with border object
      Vector2f &node_velocity, std::vector<int> &collision, const int b);
  void Friction(Vector2f &Vi_fri, // Apply friction with border object
                const Vector2f &Vi_col, const Vector2f &Vi);
